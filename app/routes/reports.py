...
def api_export_gstr1():
    ...
    try:
        ...
        buf.seek(0)
        return send_file(buf,as_attachment=True,download_name=f"GSTR1_{year}_{month}.xlsx",
                         mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    except Exception as ex:
        db.session.rollback()
        return jsonify(success=False, message=str(ex)), 500
...
def api_export_year_audit():
    ...
    try:
        ...
        return send_file(
            buf,
            as_attachment=True,
            download_name=f"year_audit_{fy_label}.xlsx",
            mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
...
def api_export_pdf():
    ...
    try:
        ...
        doc.build(story)
        return send_file(out_path,as_attachment=True,download_name=f"erp_report_{s}_to_{e}.pdf",
